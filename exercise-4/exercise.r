...
movie.no.spaces <- gsub(" ", "+", fav.movie)
...
Director <- function(movie.dataf){
  answer <- paste(movie.dataf$Title, "was directed by", movie.dataf$Director)
  return(answer)
}
...
movie.list <- c("Toy Story", "Good Will Hunting", "The Departed", "Forrest Gump", "Frozen")

# Remove spaces from each movie title
no.spaces.movie.list <- gsub(" ", "+", movie.list) 

# Prepare this list to be passed in as parameters for the API
parameter <- paste0("t=", no.spaces.movie.list, "&r=json")
